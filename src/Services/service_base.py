...
class ServiceBase(metaclass=ABCMeta):
    ...
    def __init__(self,
        ...
        """
        self.__inbound_queue: mp.Queue[QueueStatus] = inbound_queue
        """
        Attribute used to hold the inbound queue, which receives messages from the GUI.
        :meta private:
        """

        self.__outbound_queue: mp.Queue[QueueStatus | AppError | float] = outbound_queue
        """
    ...
    def _queue_listener(self) -> None:
        ...
        """
        while True:
            msg: QueueStatus = self.__inbound_queue.get()
            if msg == QueueStatus.INTERRUPT:
                self._stop_event.set()
                break

    def _send_msg(self,
                  msg: float | QueueStatus | AppError) -> None:
